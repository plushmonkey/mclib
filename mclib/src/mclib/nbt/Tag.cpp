...
std::string to_string(mc::nbt::TagType type) {
  static const std::array<std::string, 13> typeStr = {
...
DataBuffer& operator<<(DataBuffer& out, const TagLongArray& tag) {
  out << (Tag&)tag;
  return out;
}
...
void TagByteArray::Write(DataBuffer& buffer) const {
  s32 length = static_cast<s32>(m_Value.length());
...
void TagIntArray::Write(DataBuffer& buffer) const {
  s32 length = static_cast<s32>(m_Value.size());
...
TagType TagLongArray::GetType() const noexcept {
...
void TagList::Write(DataBuffer& buffer) const {
  ...
  s32 size = (m_ListType != TagType::End) ? static_cast<s32>(m_Tags.size()) : 0;
...
void TagList::CopyOther(const TagList& rhs) {
  ...
  for (std::size_t i = 0; i < m_Tags.size(); ++i) {
    ...
    } else if (m_ListType == TagType::LongArray) {
      m_Tags[i] = std::make_shared<TagLongArray>(*(TagLongArray*)tag.get());
    }
  }
}
...
void TagCompound::CopyOther(const TagCompound& rhs) {
  ...
  for (std::size_t i = 0; i < m_Tags.size(); ++i) {
    ...
    if (type == TagType::Byte) {
      ...
      newTag = std::make_shared<TagLongArray>(*(TagLongArray*)tag.get());
    }
...
DataBuffer& operator>>(DataBuffer& in, TagLongArray& tag) {
  in >> (Tag&)tag;
  return in;
}
