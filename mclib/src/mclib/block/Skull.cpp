...
bool Skull::ImportNBT(nbt::NBT* nbt) {
    ...
    if (ownerTag) {
        ...
        if (uuidTag)
            m_OwnerUUID = UUID::FromString(uuidTag->GetValue());
        ...
        if (propertiesTag) {
            ...
            if (texturesTag) {
                for (auto iter = texturesTag->begin(); iter != texturesTag->end(); ++iter) {
                    nbt::TagCompound* textureCompound = dynamic_cast<nbt::TagCompound*>(iter->get());

                    if (!textureCompound) continue;

                    auto signatureTag = textureCompound->GetTag<nbt::TagString>(L"Signature");
                    auto valueTag = textureCompound->GetTag<nbt::TagString>(L"Value");
