...
enum class TagType { End, Byte, Short, Int, Long, Float, Double, ByteArray, String, List, Compound, IntArray, LongArray };
...
public:
  ...
  MCLIB_API TagLongArray(std::wstring name, std::vector<s64> val) : Tag(name), m_Value(val) { }
  MCLIB_API TagLongArray(std::string name, std::vector<s64> val) : Tag(name), m_Value(val) { }
