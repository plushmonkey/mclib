...
BlockPlacer::BlockPlacer(mc::protocol::packets::PacketDispatcher* dispatcher, mc::core::Client* client, mc::util::PlayerController* pc, mc::world::World* world)
...
void BlockPlacer::HandlePacket(mc::protocol::packets::in::WindowItemsPacket* packet) {
    auto slots = packet->GetSlots();
    m_HeldItem = slots[36];
}

void BlockPlacer::HandlePacket(mc::protocol::packets::in::SetSlotPacket* packet) {
    ...
    if (m_HeldItem.GetItemId() != -1) {
        ...
        if (!block || block->GetType() == 0) {
            ...
            using namespace mc::protocol::packets::out;
            {
                ...
                PlayerDiggingPacket packet(status, m_Target + mc::Vector3i(0, 1, 0), mc::Face::West);
